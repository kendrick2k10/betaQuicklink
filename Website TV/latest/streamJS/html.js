...
const title = urlParams.get("description").match(/((^[^]+?:)\s([^]+?\:)|(^[^]+?:))/)[0].length < 100 ? urlParams.get("description").match(/((^[^]+?:)\s([^]+?\:)|(^[^]+?:))/)[0]: urlParams.get("description").match(/^[^]+?:/)[0]
        
const actors = urlParams.get("description").match(/(With[^])[^]+?\./)[0].replace(/with/i, "").trim()

const sypnosis = urlParams.get("description").match(/((With[^])[^]+?\.)([^]+$)/)[0].replace(/with[^]+?\./i, " ").trim()


const director = urlParams.get("description").match(/((^[^]+?:)\s([^]+?\:)|(^[^]+?:))\s((Directed\sby|Created\sby)[^]+?\.)/)[0].replace(/(^[^]+?:)\s([^]+?\:)|(^[^]+?:)/, "").trim().replace(/Directed\sby\s|Created\sby\s/, " ")
        ...
        <div class="card border-0 mb-3 shadow">
            ...
            <div id="watch" class="d-none row g-0 bg-dark" style="">
                    <video
                        ...
                        data-setup="{}"
                    >
                    <track kind="captions" src="../subtitles/ss.vtt" srclang="en" label="English" default>
                        <p class="vjs-no-js">
